...
class Warehouse_model extends CI_Model
{
	...
	public function getProductionRequestDetailApprove($Production_plan)
	{
		$query = "
			...
			LEFT JOIN production_plan p ON p.Production_plan = a.Production_plan 
			LEFT JOIN production_plan_detail pd ON pd.Production_plan = p.Production_plan AND pd.Id_material = a.Id_material 
			WHERE a.Production_plan = ?  and a.status = 1
		";
		// print_r($query);die;
		return $this->db->query($query, [$Production_plan])->result_array();
	}
	...
	{
		$this->db->select('Id_storage, Sloc, space_now, space_max');  // Updated column name
	...
	{
		$this->db->select('Id_material, Material_desc, Material_need, status');
	...
	public function getQualityRequestApprove($Id_material, $action)
	{
		// Set status based on the action (1 for approve, 2 for reject)
		$status = ($action == 'approve') ? 1 : 2;

		// Update the status in the quality_request table
		$this->db->set('status', $status);
		$this->db->where('Id_material', $Id_material);
		$this->db->update('quality_request');
