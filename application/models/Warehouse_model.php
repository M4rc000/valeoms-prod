...
class Warehouse_model extends CI_Model
{
    ...
    {
        // Query to get the list of storage locations (SLocs)
        $query = $this->db->get('storage');  // Assuming 'storage' is the table name
        return $query->result_array();  // Return the results as an array
    }

    public function get_receiving_by_box_id($box_id)
    {
        $this->db->select('id_material, material_desc, qty, uom');  // Ensure id_material is selected
        $this->db->from('receiving_material');
...
AND p.status COLLATE utf8mb4_general_ci = 'REJECTED';
    ...
    public function getQualityRequestDetail($id_request)
    {
        $query = " SELECT a.*, b.Id_material, b.Material_desc from quality_request_detail a LEFT JOIN quality_request b on b.Id_request = a.Id_Request  where a.Id_request = ? ";
        // print_r($query);die;
        return $this->db->query($query, [$id_request])->result_array();
    }
