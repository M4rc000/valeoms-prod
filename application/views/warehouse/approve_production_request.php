                ...
                <div class="row ps-2">
                    ...
                    <div class="col-3">
                        ...
                        <input type="text" class="form-control material_need_<?= $pr['Id_material'] ?>"
                            value="<?= $pr['Qty'] ?>" step="any" readonly>
                    </div>
                </div>
                <hr>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(data, function(index, box) {
                // Menggunakan total_qty_real dari material yang sesuai dalam box
            ...
            $boxSelect.on('change', function() {
                ...
                // Cek apakah box sudah dipilih sebelumnya
                ...
                // Ambil total_qty_real dari material tertentu dalam box
                var totalQty = selectedOption.data('total-qty');
                $(`.qty_on_sloc_${rowIndex}_${idMaterial}`).val(
                    totalQty); // Masukkan ke input qty_on_sloc
                $(`.qty_need_${rowIndex}_${idMaterial}`).attr('max',
                    totalQty); // Set batas maksimum pada qty_need
            });
        },
...
function setPR(rowIndex, id_material) {
    ...
    var qtyOnSlocValue = parseFloat($(`.qty_on_sloc_${rowIndex}_${id_material}`).val());
    ...
    // Validasi untuk memastikan SLoc, Box, dan Qty Need diisi
    if (!slocValue) {
        Swal.fire({
            icon: 'error',
    ...
    if (!qtyNeedValue || qtyNeedValue <= 0) {
        Swal.fire({
            icon: 'error',
    ...
    // Validasi agar qty_need tidak melebihi qty_on_sloc
    if (qtyNeedValue > qtyOnSlocValue) {
        Swal.fire({
            icon: 'error',
            text: 'Quantity Need cannot exceed the available quantity on SLoc.'
        });
        return;
    }

    // Kurangi qty_on_sloc secara real-time
    var newQtyOnSlocValue = qtyOnSlocValue - qtyNeedValue;
    $(`.qty_on_sloc_${rowIndex}_${id_material}`).val(newQtyOnSlocValue.toFixed(3)); // Format desimal jika perlu

    // Push data ke dalam materialSlocArray
    materialSlocArray.push({
        sloc: slocValue,
        box: boxValue,
        qty_need: qtyNeedValue,
    ...
    // Nonaktifkan tombol setPR dan aktifkan tombol unsetPR
    ...
    // Kalkulasi total qty_need dalam array
    var totalQtyNeed = materialSlocArray.reduce((sum, item) => sum + parseFloat(item.qty_need), 0);
    var materialNeed = parseFloat($(`.material_need_${id_material}`).val());

    // Validasi untuk menonaktifkan plus-row jika jumlah sudah tercapai
    if (materialNeed > totalQtyNeed) {
        $('.plus-row').prop('disabled', false);
    } else {
        $('.plus-row').prop('disabled', true); // Disable button jika totalQty sudah sesuai
    }

    console.log('Total Qty Need:', totalQtyNeed, 'Material Need:', materialNeed);
}
...
function unsetPR(rowIndex, id_material) {
    ...
    // Kembalikan kuantitas awal secara real-time
    var newQtyOnSlocValue = qtyOnSlocValue + qtyNeedValue; // Tambah qty_need kembali ke qty_on_sloc
    $(`.qty_on_sloc_${rowIndex}_${id_material}`).val(newQtyOnSlocValue);

    // Kembalikan kuantitas di database dengan AJAX
    $.ajax({
        url: '<?php echo base_url('warehouse/return_material_qty'); ?>',
        method: 'POST',
        data: {
            sloc: slocValue,
            box: boxValue,
        ...
        success: function(response) {
            var data = JSON.parse(response);
            if (data.status) {
                // Jika berhasil, ubah tombol unset kembali ke set
                ...
                // Hapus item dari materialSlocArray
                ...
                Swal.fire({
                    icon: 'error',
                    text: 'Error in returning quantity to storage!'
                });
            }
        },
        error: function(xhr, status, error) {
...
function savePRDetail(id_material) {
    if (materialSlocArray.length == 0) {
        Swal.fire({
            icon: 'error',
            text: 'Please Complete the process!'
        });
        return;
    }

    // Kalkulasi total qty_need dalam array
    var totalQtyNeed = materialSlocArray.reduce((sum, item) => sum + parseFloat(item.qty_need), 0).toFixed(3);
    var materialNeed = parseFloat($(`.material_need_${id_material}`).val()).toFixed(3);

    console.log("Total Qty Need:", totalQtyNeed);
    console.log("Material Need:", materialNeed);

    // Periksa apakah materialNeed sama dengan totalQtyNeed dengan presisi desimal
    if (parseFloat(materialNeed) !== parseFloat(totalQtyNeed)) {
        Swal.fire({
            icon: 'error',
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.status) {
                    Swal.fire({
                        icon: 'success',
                        text: 'Save production request detail is successful.'
                    }).then(() => {
